...
public class ClientThread extends Thread {
    ...
    public ClientThread(Context context) {
        ...
        sharedPreferences = context.getSharedPreferences("ip&port", Context.MODE_PRIVATE);
        ip = sharedPreferences.getString("ip", "172.20.10.13");
                    ...
                    if (FrameDataLen + FrameStart < DataCache_End) {
                        ...
                        if (IsFoundFullFrame) {
                            if (!IsValidFrame) {
                                ...
                                stringBuilder.append("接收CRC：0x" + Byte2HexStr(CheckCRC) + "   ");
                                stringBuilder.append("计算CRC：0x" + Byte2HexStr(CalCRC) + "\n");
                                stringBuilder.append("数据帧：\n");
                                for (j = 0; j < DecPacketLen; j++) {
                                    stringBuilder.append(Byte2HexStr(DecPacket[j]));
                                }
                                stringBuilder.append("\n");
                            } else {
                                // 对译码后的数据包进行解析
                                ...
                                switch (FrameType) {
                                    ...
                                    case (byte) 0x01: // 发送节点地址，父节点地址，协议版本
                                        ...
                                        for (j = 0; j < 2; j++) {
                                            stringBuilder.append(Byte2HexStr(DecPacket[4 + j]));

                                        }

                                        StrC_NA = Byte2HexStr(DecPacket[4]) + Byte2HexStr(DecPacket[5]);
                                        // nodeinfo.setId(Integer.toHexString(Integer.parseInt(StrC_NA,
                                        // 16)));
                                        // nodeinfo.setId(StrC_NA);
                                        stringBuilder.append("\n" + "父节点地址：0x");
                                        for (j = 0; j < 2; j++) {
                                            stringBuilder.append(Byte2HexStr(DecPacket[6 + j]));
                                        }

                                        StrP_NA = Byte2HexStr(DecPacket[6]) + Byte2HexStr(DecPacket[7]);
                                        // nodeinfo.setParentId(StrP_NA);
                                        stringBuilder.append("\n" + "协议栈版本:");
                                        if (Byte2HexStr(DecPacket[8]).equals("10")) {
                                            stringBuilder.append("ZigBee 2007");
                                            StrPROFILE = "ZigBee 2007";
                                        } else if (Byte2HexStr(DecPacket[8]).equals("11")) {
                                            stringBuilder.append("ZigBee 2007 pro");
                                            StrPROFILE = "ZigBee 2007 pro";
                                        } else {
                                            stringBuilder.append("##Error##协议栈版本错误，请校验");
                                        }
                                        break;
                                    ...
                                    case (byte) 0x02: // 节点综合信息报告
                                        ...
                                        stringBuilder.append("\n" + "所在网络ID:0x");
                                        for (j = 0; j < 2; j++) {
                                            stringBuilder.append(Byte2HexStr(DecPacket[8 + j]));
                                        }
                                        StrC_PANID = "0x"
                                        ...
                                        if (Byte2HexStr(DecPacket[10]).equals("10")) {
                                            stringBuilder.append("ZigBee 2007");
                                            StrPROFILE = "ZigBee 2007";
                                        } else if (Byte2HexStr(DecPacket[10]).equals("11")) {
                                            stringBuilder.append("ZigBee 2007 pro");
                                            StrPROFILE = "ZigBee 2007";
                                        } else {
                                            stringBuilder.append("##Error##协议栈版本错误，请校验");
                                        ...
                                        stringBuilder.append("\n" + "电源电压:" + floatNum.format(Volatage) + "V");
                                        // nodeinfo.setInfo("电源电压:" +
                                        // floatNum.format(Volatage) + "V");
                                        StrPOWER_VOL = floatNum.format(Volatage) + "V";

                                        stringBuilder.append("\n" + "版本信息:");
                                        StrC_VER = "V." + Byte2HexStr(DecPacket[24]);
                                        System.out.println("WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW");

                                        GetSensorInfoByType(DecPacket, 11, stringBuilder, nodeinfo);
                                        addANode(nodeinfo);
                                        break;
                                    ...
                                    case (byte) 0x03: // 查询传感器数据响应
                                        ...
                                        GetSensorInfoByType(DecPacket, 6, stringBuilder, nodeinfo1);
                                        addANode(nodeinfo1);
                                        break;
                                    case (byte) 0x04: // 节点加入网络报告
                                        ...
                                        StrP_NA = "0x" + Byte2HexStr(DecPacket[14]) + Byte2HexStr(DecPacket[15]);
                                        stringBuilder.append("\n父节点地址 :" + StrP_NA);
                                    ...
                                    case (byte) 0x11: // 查询节点信息响应
                                        ...
                                        // nodeinfo.setId(Integer.toHexString(Integer.parseInt(Byte2HexStr(DecPacket[4])
                                        // + Byte2HexStr(DecPacket[5]), 16)));
                                        GetNodeInfoByType(DecPacket, 6, stringBuilder);
                                    ...
                                    default:
                                        ...
                                        stringBuilder.append("FrameType = " + FrameType + "\n");
                                        for (j = 0; j < DecPacketLen; j++) {
                                            stringBuilder.append(Byte2HexStr(DecPacket[j]));
                                        }
                                        stringBuilder.append("\n");
    ...
    // 查询传感器数据
    private void GetSensorInfoByType(byte[] DecPacket, int p, StringBuilder stringBuilder, NodeInfo node) {
        // p为Type的下标，i为Data的下标
        int i = p + 1;
        // 传感器的数据
        short SensorData = 0;
        ...
        // Z轴数据(加速度传感器，陀螺仪)
        float zVal = 0.0f;
        ControlInterface con = new ControlInterface();
        String str;
        stringBuilder.append("\n" + "����������:");
        ...
        switch (DecPacket[p]) {
            ...
            case (byte) 0x0b: // SHT10温湿度传感器(高精度)
                ...
                SensorData = (short) (((DecPacket[i] & 0xff) << 8) + ((DecPacket[i + 1]) & 0xff));
                Humidity = (float) (-2.0468 + 0.0367 * SensorData + (-1.5955E-6) * SensorData * SensorData);
                Humidity = (float) ((Temperature - 25) * (0.01 - 0.00008 * Humidity) + Humidity);
                StrSENSOR_DATA = "温度:" + floatNum.format(Temperature) + "℃\n" + "湿度:" + floatNum.format(Humidity) + "%";
                values[0] = Temperature;
                values[1] = Humidity;
                node.setValues(values);
                Log.i("info", StrSENSOR_DATA);
                break;
            ...
            case (byte) 0x13:
                ...
                int lightStrength = (int) (((DecPacket[i] & 0xff) << 8) + ((DecPacket[i + 1]) & 0xff));
                StrSENSOR_DATA = "光强：" + lightStrength + "lx";
                node.setValue(lightStrength);
                break;
            case (byte) 0x14:
            ...
            case (byte) 0x15:
                ...
                SensorData = (short) (((DecPacket[i] & 0xff) << 8) + ((DecPacket[i + 1]) & 0xff));
                Volatage = (float) ((float) SensorData * 3.3 / 2048);
                StrSENSOR_DATA = String.format("%.2f", (26.757 * Math.pow(Volatage, -1.236))) + "cm";
                node.setValue(Volatage );
                break;
            case (byte) 0x16:
    ...
    private void GetNodeInfoByType(byte[] DecPacket, int p, StringBuilder stringBuilder) {
        int j = 0;
        // p为Type的下标，i为Data的下标
        int i = p + 1;
        // 节点运行时间
        ...
        switch (DecPacket[p]) {
            ...
            case (byte) 0x04: // 节点PAN ID
                stringBuilder.append("\n" + "查询节点PAN ID:");
                StrC_PANID = "0x" + Byte2HexStr(DecPacket[i]) + Byte2HexStr(DecPacket[i + 1]);
            ...
            case (byte) 0x07: // 节点采集时间ms
                stringBuilder.append("\n" + "查询节点采集时间:");
                GatherTime = ((DecPacket[i] & 0xff) << 8) + (DecPacket[i + 1] & 0xff);
            ...
            case (byte) 0x0a: // 节点PWM参数
                stringBuilder.append("\n" + "查询节点PWM参数:");
                PWMFre = ((DecPacket[i + 1] & 0xff) << 8) + ((DecPacket[i + 2]) & 0xff);
...
//          将温湿度传感器转换为两个传感器
            if (node.getType() == 0x0b) {
                NodeInfo node_t = (NodeInfo) node.clone();
                NodeInfo node_h = (NodeInfo) node.clone();
//                温度节点
                node_t.setType((byte) 0x1b);
                node_t.setValue(node.getValues()[0]);
//                湿度节点
                node_h.setValue(node.getValues()[1]);
                node_h.setType((byte) 0x2b);
                nodelist.add(node_t);
                nodelist.add(node_h);
            } else {
                nodelist.add(node);
        ...
        if (node.getType() == 0x17) {
            Message msg = new Message();
            msg.what = 0x2222;
            msg.obj = node;
            if (ControlInterface.handler != null && ControlInterface.currentUiName.equals("I/O控制")) {

                if (ControlInterface.nodeinfo != null && ControlInterface.nodeinfo.getId().equals(node.getId())) ControlInterface.handler.sendMessage(msg);

            }
        }
        if (node.getType() == 0x08) {
            Message msg = new Message();
            msg.what = 0x2223;
            msg.obj = node;
            if (ControlInterface.handler != null && ControlInterface.currentUiName.equals("马达")) ControlInterface.handler.sendMessage(msg);
        }
        //超时删除
        Iterator<NodeInfo> litrator = nodelist.iterator();
        while (litrator.hasNext()) {
            if (System.currentTimeMillis() - litrator.next().getLastRecordTime() > 10000) {
                litrator.remove();
            }
        }
        Collections.sort(nodelist, new Comparator<NodeInfo>() {
            @Override
            public int compare(NodeInfo nodeInfo, NodeInfo t1) {
                return nodeInfo.getId().compareTo(t1.getId());
            }
        });
    ...
    private boolean replaceANodeInList(List<NodeInfo> list, NodeInfo newNode) {
        Boolean changed = false;
        if (list != null && list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                NodeInfo nodeInList = list.get(i);
//                如果有id相同的node
                if (nodeInList.getId().equals(newNode.getId())) {
                    newNode.setType(nodeInList.getType());
//                    如果是温湿度传感器
                    if (newNode.getType() == 0x1b) {
                        newNode.setValue(newNode.getValues()[0]);
                    } else if (newNode.getType() == 0x2b) {
                        newNode.setValue(newNode.getValues()[1]);
                    }
                    list.set(i, (NodeInfo) newNode.clone());
                    changed = true;
                }
            }
        }
    ...
    public String remoteControl(byte[] DecPacket, int i) {
        ...
        if ("0x00".equals(str)) {
            ...
            deviceName = "空调";
        } else if ("0x2b".equals(str)) {
            deviceName = "除湿器";
        } else if ("0x13".equals(str)) {
            deviceName = "电灯";
        } else if ("0x15".equals(str)) {
            deviceName = "门";
